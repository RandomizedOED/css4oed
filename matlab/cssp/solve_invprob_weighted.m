function [x, r, obj, data_m, pr_m] = solve_invprob_weighted(F, Gn, Gp_inv, ...
                                         y, mu, alpha, S, Sw, ...
                                         solve_cg, maxiters, precon)
% SOLVE_INVPROB_WEIGHTED Solves the linear Bayesian inverse problem given by
...
% The method also optionally accepts two row sampling matrices S, Sw to solve 
% the problem after weighted sensor placement via OED. Please refer to the 
% paper for details.
...
%  Sw       - Weighted row sampler matrix (optional)
...
%  pr_m   - Prior misfit term
  ...
  [sel_F, ~, sel_y] = form_selops(F, Gn, y, Sw);
  ...
  if (solve_cg) % Solve via CG
    ...
    [x, exit_code] = pcg(A, b, 1e-6, maxiters, M);
    if (exit_code > 0)
